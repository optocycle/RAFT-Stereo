...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root="datasets/Middlebury", split="F"):
        ...
        if split == "2014":  # datasets/Middlebury/2014/Pipes-perfect/im0.png
            ...
            lines = list(
                map(osp.basename, glob(os.path.join(root, "MiddEval3/trainingF/*")))
            )
            lines = list(
                filter(
                    lambda p: any(
            ...
            image1_list = sorted(
                [
                    os.path.join(
                        root, "MiddEval3", f"training{split}", f"{name}/im0.png"
                    )
                    for name in lines
                ]
            )
            image2_list = sorted(
                [
                    os.path.join(
                        root, "MiddEval3", f"training{split}", f"{name}/im1.png"
                    )
                    for name in lines
                ]
            )
            disp_list = sorted(
                [
                    os.path.join(
                        root, "MiddEval3", f"training{split}", f"{name}/disp0GT.pfm"
                    )
                    for name in lines
                ]
            )
