    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            if self.args.shared_backbone:
                # NOTE this was changed to support batching
                *cnet_list1, x1 = self.cnet(
                    image1, dual_inp=False, num_layers=self.args.n_gru_layers
                )
                *cnet_list2, x2 = self.cnet(
                    image2, dual_inp=False, num_layers=self.args.n_gru_layers
                )
                fmap1 = self.conv2(x1)
                fmap2 = self.conv2(x2)
